...
function _createExam() {
  try {
    ...
    $("#regTitle, #examAbbr, #regPix, #incentives, #statusId").removeClass(
      "issue"
    );
    $(
      "#issue_regTitle, #issue_examAbbr, #issue_regPix, #issue_incentives, #issue_statusId"
    ).html("");

    if (!regTitle) {
      $("#regTitle").addClass("issue");
      $("#issue_regTitle").html(
        "USER ERROR! Kindly Provide exam name to continue"
      );
      issueCount++;
    }
    ...
    if (!examAbbr) {
      ...
      $("#issue_examAbbr").html(
        "USER ERROR! Kindly Provide exam abbreviation to continue"
      );
      issueCount++;
    }
    ...
    if (!incentives) {
      ...
      $("#issue_incentives").html(
        "USER ERROR! Kindly provide exam incentives to continue"
      );
      issueCount++;
    }
    ...
    _showCustomConfirm({
      ...
      title: "Are you sure?",
      message:
        "Are you sure you want to create a new exam? This action is irreversible.",
      alertType: "warning",
      falseActionBtn: true,
    });
  } catch (error) {
...
function _createExamCallback(form) {
  let getEachExamSession = JSON.parse(
    sessionStorage.getItem("getEachExamSession")
  );
  ...
  let callUrl = getEachExamSession?.publishId
    ? `${endPoint}/admin/publish/exams/update-exam?pageCategoryId=${pageCategory.examCategory}&publishId=${getEachExamSession?.publishId}`
    : `${endPoint}/admin/publish/exams/create-exam?pageCategoryId=${pageCategory.examCategory}`;
...
function _fetchEachExam(pageCategoryId, publishId) {
  ...
  try {
    $.ajax({
      ...
      error: function (textStatus, errorThrown) {
        ...
        _showCustomConfirm({
          title: "Connection Error!",
          message:
            "An error occurred while fetching staff details! Please try again.",
          alertType: "error",
          trueActionBtnText: "OK, Retry",
        });
      },
    ...
    _showCustomConfirm({
      title: "Unexpected Error!",
      message:
        "An unexpected error occurred while fetching staff details! Please try again.",
      alertType: "error",
      trueActionBtnText: "OK, Retry",
    });
  }
