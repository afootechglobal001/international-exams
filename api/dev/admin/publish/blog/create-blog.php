...
<?php
    ...
    validateEmptyField($regTitle, 'BLOG TITLE');
    ...
    if ($titleCountQuery>0){ /// start if 2
        $response = [
            'response'=> 101,
            'success'=> false,
            'message' => "This Blog with name ('$regTitle') is already in use. Please try another Name."
        ];

        $alertDetail="BLOG REGISTRATION ATTEMPT FAILED: A staff whose name - ($loginStaffFullname) - (ID: $loginStaffId) attempted to register a new blog with a name ($regTitle) that is already in use.";	
        goto end;
    }
        ...
        if (!in_array(($extension), $allowedExts)) {
            ...
            (`pageCategoryId`, `publishId`, `blogCatId`, `regTitle`, `regPix`, `statusId`, `createdBy`, `createdTme`, `updatedTime`) VALUES
            ('$pageCategoryId', '$publishId', '$blogCatId', '$regTitle', '$regPix', '$statusId', '$loginStaffId', NOW(), NOW())")or die (mysqli_error($conn));
            ...
            $response = [
                ...
                'success'=> true,
                'regPix' => $regPix,
                'message'=> "BLOG CREATED SUCCESFFULLY!",
                'data' => array() // Initialize the data array
            ];

            $alertDetail = "BLOG CREATED SUCCESSFULLY: $pageCategoryName was created successfully by $loginStaffFullname (ID: $loginStaffId). DETAILS: Title: $regTitle | Blog Category: $blogCatId | ID: $publishId.";
