...
<?php
    ...
    $titleQuery=mysqli_query($conn,"SELECT faqQuestion FROM PUBLISH_TAB WHERE faqQuestion='$faqQuestion' AND pageCategoryId='$pageCategoryId'") or die (mysqli_error($conn));
    ...
    if ($titleCountQuery>0){ /// start if 2
        ...
        $alertDetail="FAQ REGISTRATION ATTEMPT FAILED: A staff whose name - ($loginStaffFullname) - (ID: $loginStaffId) attempted to register a new FAQ with a question ($faqQuestion) that is already in use.";	
        goto end;
    }
        ...
        (`pageCategoryId`, `publishId`, `faqCatId`, `faqQuestion`, `faqAnswer`, `statusId`, `createdBy`, `createdTme`, `updatedTime`) VALUES
        ('$pageCategoryId', '$publishId', '$faqCatId', '$faqQuestion', '$faqAnswer', '$statusId', '$loginStaffId', NOW(), NOW())")or die (mysqli_error($conn));
        ...
        $response = [
            'response'=> 200,
            'success'=> true,
            'message'=> "FAQ CREATED SUCCESFFULLY!",
            'data' => array() // Initialize the data array
        ];

        $alertDetail = "FAQ CREATED SUCCESSFULLY: $pageCategoryName was created successfully by $loginStaffFullname (ID: $loginStaffId). DETAILS: Question: $faqQuestion | Answer: $faqAnswer | ID: $publishId.";
