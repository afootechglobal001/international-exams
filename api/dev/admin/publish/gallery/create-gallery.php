...
<?php
    ...
    validateEmptyField($regTitle, 'GALLERY TITLE');
    ...
    if ($titleCountQuery>0){ /// start if 2
        ...
        $alertDetail="GALLERY REGISTRATION ATTEMPT FAILED: A staff whose name - ($loginStaffFullname) - (ID: $loginStaffId) attempted to register a new gallery with a name ($regTitle) that is already in use.";	
        goto end;
    }
        ...
        if (!in_array(($extension), $allowedExts)) {
            ...
            (`pageCategoryId`, `publishId`, `regTitle`, `regPix`, `statusId`, `createdBy`, `createdTme`, `updatedTime`) VALUES
            ('$pageCategoryId', '$publishId', '$regTitle', '$regPix', '$statusId', '$loginStaffId', NOW(), NOW())")or die (mysqli_error($conn));
            ...
            $alertDetail = "GALLERY CREATED SUCCESSFULLY: $pageCategoryName was created successfully by $loginStaffFullname (ID: $loginStaffId). DETAILS: Title: $regTitle | ID: $publishId.";

            // Fetch branch details ///
            $select="SELECT a.pageCategoryId, a.publishId, a.regTitle, a.regPix, a.statusId, a.createdBy, a.createdTme, a.updatedTime, b.statusName FROM PUBLISH_TAB a, SETUP_STATUS_TAB b WHERE a.statusId=b.statusId AND a.pageCategoryId='$pageCategoryId'";
