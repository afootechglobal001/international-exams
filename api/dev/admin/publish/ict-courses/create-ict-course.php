...
<?php
    ...
    $subTitle =trim(str_replace("'", "\'", $_POST['subTitle']));
    ...
    validateEmptyField($subTitle, 'SUB TITLE');
    ...
    if ($titleCountQuery>0){ /// start if 2
        $response = [
            'response'=> 101,
            'success'=> false,
            'message' => "This ICT Course with name ('$regTitle') is already in use. Please try another Name."
        ];

        $alertDetail="ICT COURSE REGISTRATION ATTEMPT FAILED: A staff whose name - ($loginStaffFullname) - (ID: $loginStaffId) attempted to register a new ICT Course with a name ($regTitle) that is already in use.";	
        goto end;
    }
        ...
        if (!in_array(($extension), $allowedExts)) {
            ...
            (`pageCategoryId`, `publishId`, `regTitle`, `subTitle`, `regPix`, `statusId`, `createdBy`, `createdTme`, `updatedTime`) VALUES
            ('$pageCategoryId', '$publishId', '$regTitle', '$subTitle', '$regPix', '$statusId', '$loginStaffId', NOW(), NOW())")or die (mysqli_error($conn));
            ...
            $response = [
                ...
                'success'=> true,
                'regPix' => $regPix,
                'message'=> "ICT COURSE CREATED SUCCESFFULLY!",
                'data' => array() // Initialize the data array
            ];

            $alertDetail = "ICT COURSE CREATED SUCCESSFULLY: $pageCategoryName was created successfully by $loginStaffFullname (ID: $loginStaffId). DETAILS: Title: $regTitle | Sub Title: $subTitle | ID: $publishId.";

            // Fetch branch details ///
            $select="SELECT a.pageCategoryId, a.publishId, a.regTitle, a.subTitle, a.regPix, a.statusId, a.createdBy, a.createdTme, a.updatedTime, b.statusName FROM PUBLISH_TAB a, SETUP_STATUS_TAB b WHERE a.statusId=b.statusId AND a.pageCategoryId='$pageCategoryId'";
