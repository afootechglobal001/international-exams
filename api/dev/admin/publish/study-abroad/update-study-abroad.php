...
<?php
    ...
    if ($titleCountQuery>0){ /// start if 4
        ...
        $alertDetail="STUDY ABROAD UPDATE ATTEMPT FAILED: A staff whose name - ($loginStaffFullname) - (ID: $loginStaffId) attempted to update study abroad with a name ($regTitle) that is already in use.";	
        goto end;
    }

        $update ="UPDATE PUBLISH_TAB SET 
            regTitle = '$regTitle',
            studyAbroadSummary = '$studyAbroadSummary',
            statusId = '$statusId',
            ...
            $response = [
                ...
                'regPix' => $regPix,
                'oldRegPix' => $oldRegPix,
                'message'=> "STUDY ABROAD UPDATED SUCCESFFULLY!",
                'data' => array() // Initialize the data array
            ];

            $alertDetail = "STUDY ABROAD UPDATED SUCCESSFULLY: $pageCategoryName was upadated successfully by $loginStaffFullname (ID: $loginStaffId). DETAILS: Title: $regTitle | Summary: $studyAbroadSummary | ID: $publishId.";

            // Fetch branch details ///
            $select="SELECT a.pageCategoryId, a.publishId, a.regTitle, a.studyAbroadSummary, a.regPix, a.statusId, a.createdBy, a.updatedBy, a.createdTme, a.updatedTime, b.statusName FROM PUBLISH_TAB a, SETUP_STATUS_TAB b WHERE a.statusId=b.statusId AND a.pageCategoryId='$pageCategoryId'";
